...
async def update_achievement_roles(guild):
    ...
    # Clear all achievement roles first
    for role in [most_deaths_role, least_deaths_role, most_adv_role, least_adv_role, 
                most_playtime_role, least_playtime_role]:
        if role:
            members_with_role = [member for member in guild.members if role in member.roles]
            for member in members_with_role:
                await member.remove_roles(role)
    ...
    if deaths_data and most_deaths_role:
        ...
        for player in most_deaths_players:
            ...
            if discord_member:
                await discord_member.add_roles(most_deaths_role)
                print(f"Assigned {MOST_DEATHS_ROLE} to {discord_member.name}")
    ...
    if deaths_data and least_deaths_role:
        ...
        if eligible_players:
            ...
            for player in least_deaths_players:
                discord_member = get_discord_user(bot, player[1])
                if discord_member:
                    await discord_member.add_roles(least_deaths_role)
                    print(f"Assigned {LEAST_DEATHS_ROLE} to {discord_member.name}")
    ...
    if advancements_data and most_adv_role:
        ...
        for player in most_adv_players:
            discord_member = get_discord_user(bot, player[1])
            if discord_member:
                await discord_member.add_roles(most_adv_role)
                print(f"Assigned {MOST_ADVANCEMENTS_ROLE} to {discord_member.name}")
    
    # Handle least advancements
    if advancements_data and least_adv_role:
        min_adv = min(advancements_data, key=lambda x: x[2])[2]
        least_adv_players = [p for p in advancements_data if p[2] == min_adv]
        for player in least_adv_players:
            discord_member = get_discord_user(bot, player[1])
            if discord_member:
                await discord_member.add_roles(least_adv_role)
                print(f"Assigned {LEAST_ADVANCEMENTS_ROLE} to {discord_member.name}")
    ...
    if playtimes_data and most_playtime_role:
        ...
        for player in most_playtime_players:
            discord_member = get_discord_user(bot, player[1])
            if discord_member:
                await discord_member.add_roles(most_playtime_role)
                print(f"Assigned {MOST_PLAYTIME_ROLE} to {discord_member.name}")
    ...
    if playtimes_data and least_playtime_role:
        ...
        if eligible_players:
            ...
            for player in least_playtime_players:
                discord_member = get_discord_user(bot, player[1])
                if discord_member:
                    await discord_member.add_roles(least_playtime_role)
                    print(f"Assigned {LEAST_PLAYTIME_ROLE} to {discord_member.name}")
    ...
    try:
        ...
        async for message in channel.history(limit=10):
            ...
            if message.author == bot.user:
                ...
                await asyncio.sleep(0.5)  # Add a short delay to avoid rate limiting
    except Exception as e:
        print(f"Error clearing previous leaderboard messages: {e}")
        ...
        # Player join - check for both bold and plain text formats
            ...
            if match:
                ...
                if minecraft_username in MINECRAFT_TO_DISCORD:
                    ...
                    status_text = f"Online: {', '.join(discord_display_names)}"
...
"""
    ...
    embed.add_field(name="Current Values", value=current_values, inline=False)
